...
入出力:
  入力: data/staging/YYYY/MMDD/single_train.csv（推奨。無い場合は *single*.csv や *_train.csv をフォールバック探索）
       必須列: is_win（1=1着, 0=その他）
  出力: {out_root}/{YYYY}/{MMDD}/proba_{test_start}_{test_end}.csv
       モデル: {model_root}/model.txt  (または --model_out)
...
def load_dataset(paths: list[Path]) -> pl.DataFrame:
    ...
    lfs = []
    for p in paths:
        # Polars 0.20.3x の scan_csv 最新引数対応
        lf = pl.scan_csv(
            str(p),
...
def fit_lgb(train_pd, feat_cols, cfg: TrainConfig) -> lgb.Booster:
    ...
    booster = lgb.train(
        params, dset, num_boost_round=cfg.num_boost_round,
...
def main():
    ...
    # 互換引数（旧YAML対応）
    ap.add_argument("--results_root",   default=None, help="互換引数: 受理して無視（単勝では未使用）")
