...
STR_FEAT_BLACKLIST = set(["racer_name","decision4"])
...
def find_csvs(integrated_root: str, start: str, end: str, name="integrated_train.csv") -> list[Path]:
    files = []
    for dd in _ymd_dirs(Path(integrated_root), start, end):
        p = dd / name
        if p.exists():
            files.append(p)
        else:
...
def load_train(paths: list[Path]) -> pl.DataFrame:
    ...
    if "rank" not in df.columns:
        raise RuntimeError("integrated_train.csv に rank(1..6) 列が必要です。")
    for c in ("hd","jcd"):
...
def build_place2_table(df: pl.DataFrame):
...
def build_place3_table(df: pl.DataFrame):
    win = df.filter(pl.col("rank")==1).select(RACE_KEY+["lane"]).rename({"lane":"first_lane"})
